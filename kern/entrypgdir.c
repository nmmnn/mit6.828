...
// 两个页目录表，这个pgdir地址是放在CR3页目录寄存器里的，里面有两个表项，下标为0的
// 表项存放的是一个页表地址，这个页表占1个页面4K，存放了0~4M的虚拟地址共1024个页面
// 对应的地址，第二个表项下标为0XF00000 >> 22，X86的地址第22位包含了页号和页内偏移，
// 剩下的高10位就是dir页目录号，对应的是内核所在的虚拟地址空间0XF00000
// 开始的4M，也是映射到同一个页表entry_pgtable的地址，所以映射到的是同一片4M物理内存
// 然后还设置了一些标志位
...
// physical page 1, etc.
// 这个页表映射从0开始的1024个页面=4M的物理内存，上面的页目录表两个表项都用的这个页表
// 所以映射到同一片内存

/*
这个页表里的页表项怎么看：正常的页表项分为 物理页地址(20位，共4M个页的物理地址) + 页的控制位(12位)
这个页表只用到了1024个个表项，对应1024个页面，所以物理页的地址只需要10位，3个十六进制就够了（0x000~0x3ff）,再加上12位控制位，一共就需要6个二进制位，
可以看到，下面的6位十六进制数字的第三位也就是低12位都是0，跟后面的标志相与就可以对相应的控制位进行置位
*/
__attribute__((__aligned__(PGSIZE)))
pte_t entry_pgtable[NPTENTRIES] = { 
	...
	0x3ff000 | PTE_P | PTE_W, // 0x3ff一共10位，2^10 = 1024个页面
};
